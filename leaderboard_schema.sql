...
WITH ranked_users AS (
  SELECT
    ...
    (SELECT COUNT(*) FROM activities WHERE user_id = user_profiles.id) AS activities_count,
...
SELECT
  ...
  (SELECT 
    COUNT(DISTINCT user_id)::float / NULLIF(COUNT(DISTINCT user_profiles.id), 0) * 100
   FROM activities 
  ...
  (SELECT 
    (COUNT(DISTINCT user_id)::float / NULLIF(COUNT(DISTINCT user_profiles.id), 0) * 100) - 
    (SELECT COUNT(DISTINCT user_id)::float / NULLIF(COUNT(DISTINCT user_profiles.id), 0) * 100
     FROM activities 
     CROSS JOIN user_profiles
...
BEGIN
  ...
  SELECT
    ...
    total_points,
    (SELECT COUNT(*) FROM activities WHERE user_id = user_profiles.id),
    current_day_streak,
    CURRENT_DATE
  FROM
    user_profiles
...
ranked_users AS (
  ...
  FROM
    user_profiles up
  LEFT JOIN
    weekly_points wp ON up.id = wp.user_id
  LEFT JOIN
    activities a ON up.id = a.user_id AND a.created_at >= DATE_TRUNC('week', CURRENT_DATE)
  WHERE
    up.is_active = true
  GROUP BY
...
ranked_users AS (
  ...
  FROM
    user_profiles up
  LEFT JOIN
    monthly_points mp ON up.id = mp.user_id
  LEFT JOIN
    activities a ON up.id = a.user_id AND a.created_at >= DATE_TRUNC('month', CURRENT_DATE)
  WHERE
    up.is_active = true
  GROUP BY
